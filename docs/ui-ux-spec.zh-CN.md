...
- 多会话状态：气泡在并发会话间每 1.8s 轮播，刚完成的会话短暂展示「已完成」；**活跃优先**——流式进行、后台任务运行中或等待后台任务期间不显示「任务失败」，失败只在该会话最新一条非 ambient 任务结算为 `failed` 时显示，出现更新的任务结算（或新一轮活动）即解除；`interrupted`/`stopped`（含用户主动停止）不算失败。mission 侧同规则：有待人工或运行中的任务时优先显示等待/运行。回归：`pet-state.test.ts`。
