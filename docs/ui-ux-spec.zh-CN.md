...
- **自建线：应用内更新已关闭，更新说明页签＝纯变更日志查看器**：启动自动检查（原 3 秒一次）、设置页与页头的「检查更新 / 立即更新」、右下角更新浮层（`UpdateToast`）全部下线，`tauri.conf.json` 的 `plugins.updater`（endpoint/pubkey）同步移除，`update/store.ts` 的 `UPDATES_DISABLED` 短路兜底（任何漏网调用不触网）；设置页「更新」变为静态「CC GUI 自建版 · vX」行（`UpdateSection.tsx`，锚点 `appVersion`、无可点按钮），状态栏版本号显示「vX · 自建版」并仍打开该页签。升级后首启的发布说明宣布保留（`announceReleaseAfterUpgrade`）；页签正文回落本地 `CHANGELOG_DATA` 里**同版本**条目（双语排序，markdown 映射在 `ReleaseNotesPane.tsx` 内），没有时明说「这个版本没有附带更新说明」——不把相邻版本的说明挂在当前版本标题下。页签是最弱的单实例面：自动弹出时不抢已在视的插件中心 / 任务工作台（`centerSurfaces` 的优先级）。回归：`UpdateSection.test.tsx`、`update/store.test.ts`、`ReleaseNotesPane.test.tsx`、`app-status-bar.test.tsx`、`builtin-search.test.tsx`。
