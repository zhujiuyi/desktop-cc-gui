...
- **自建线：应用内更新已关闭，更新说明页签＝纯变更日志查看器**：启动自动检查（原 3 秒一次）、设置页与页头的「检查更新 / 立即更新」、右下角更新浮层（`UpdateToast`）全部下线；`tauri.conf.json` 的 `plugins.updater` 收敛为**最小骨架**（`pubkey` 必填 ＋ `endpoints: []`——Tauri 插件配置段不可缺，删整段会让注册中的 updater 插件启动即 panic），`update/store.ts` 的 `UPDATES_DISABLED` 短路兜底（任何漏网调用不触网）；设置页「更新」变为静态「CC GUI 自建版 · vX」行（`UpdateSection.tsx`，锚点 `appVersion`、无可点按钮），状态栏版本号显示「vX · 自建版」并仍打开该页签。升级后首启的发布说明宣布保留（`announceReleaseAfterUpgrade`）；页签正文回落本地 `CHANGELOG_DATA` 里**同版本**条目（双语排序，markdown 映射在 `ReleaseNotesPane.tsx` 内），没有时明说「这个版本没有附带更新说明」——不把相邻版本的说明挂在当前版本标题下。页签是最弱的单实例面：自动弹出时不抢已在视的插件中心 / 任务工作台（`centerSurfaces` 的优先级）。回归：`UpdateSection.test.tsx`、`update/store.test.ts`、`ReleaseNotesPane.test.tsx`、`app-status-bar.test.tsx`、`builtin-search.test.tsx`。
...
- 多会话状态：气泡在并发会话间每 1.8s 轮播，刚完成的会话短暂展示「已完成」。任务失败是**三段式**：**① 失败即显示**——失败结算后置顶暂显 ~5.4s（与「已完成」同节奏；即便本轮仍在进行也先露脸）；**② 轮内让位**——暂显过后回到该会话的实时状态（后续任务/思考/等待），失败不锁屏；**③ 收尾持续**——会话结束（无流式、无运行中任务、无等待）且失败之后该会话再无任何活动（无更新的任务结算、无更晚的消息）时，失败作为本轮最后一个状态持续显示，直到出现新一轮活动。`interrupted`/`stopped`（含主动停止）不算失败；mission 侧维持活跃优先。回归：`pet-state.test.ts`。
...
| 版本 | 时间 | 内容 |
|---|---|---|
| v0.60 | 2026-09-25 | 宠物失败状态改三段式：失败即显（置顶暂显 5.4s）→ 轮内让位后续状态（不锁屏）→ 收尾时仅当失败是该轮最后状态才持续显示；`interrupted`/`stopped` 不算失败的口径不变（§4.3 同步） |
| v0.59 | 2026-09-25 | 自建线定制批：应用内更新彻底关闭（启动自动检查/设置页与页头按钮/更新浮层/`plugins.updater` endpoint 全下线，设置页与状态栏「自建版」标识，说明页签变纯变更日志查看器）；后台任务面板新增「清空已结束」与运行中任务行内「停止任务」（`stop_task` 控制帧）；进程折叠行跑马灯/流光覆盖纯工具态、展开时底部吸附「收起过程」按钮；宠物状态活跃优先、失败不再锁死会话（§3、§4.3 同步） |
